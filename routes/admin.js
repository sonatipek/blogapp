...
router.post('/blog/create', upload.single('image'), async (req, res) => {
    const title = req.body.title,
        blog_sum = req.body.blog_summary,
        desc = req.body.desc,
    ...
    try {
        await db.execute("INSERT INTO blogs(title, blog_sum ,descr, img, is_shown_homepage, category) VALUES(?,?,?,?,?,?)", [
            title, blog_sum, desc, image, isShownHomepage, category, 
        ]);
...
router.post('/blogs/:blogid', upload.single('image'), async (req, res) => {
    ...
    try {
        if (blogID === blogIDServer) {
            await db.execute('UPDATE blogs SET title=?, blog_sum=?, descr=?, img=?, is_shown_homepage=?, category=? WHERE blogid = ?', [
                title, blog_sum, desc, image, isShownHomepage, category, blogID
            ])
...
>>>>>>> e1cc4f6cb4e909b8fe7adc3034875226c04e20f2
